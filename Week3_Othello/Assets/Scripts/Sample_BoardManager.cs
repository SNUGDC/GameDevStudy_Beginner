...
public class Sample_BoardManager : MonoBehaviour
{
    ...
    private void CreateBoard()
    {
        ...
        for (int i = 0; i < boardSize; i++)
        {
            for (int j = 0; j < boardSize; j++)
            {
                ...
                board[i, j] = Instantiate(boardPrefab, spawnPos, Quaternion.identity, gameObject.transform);

                board[i, j].GetComponent<Sample_Board>().myX = i;
                board[i, j].GetComponent<Sample_Board>().myY = j;
                board[i, j].GetComponent<Sample_Board>().myState = -1;
            }
        }
    ...
    {
        board[x, y].GetComponent<Sample_Board>().myState = state;
    }
    ...
    private List<Vector2> GetChangeList(int x, int y)
    {
        ...
        if (board[x, y].GetComponent<Sample_Board>().myState != -1)
        {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            while (board[targetX, targetY].GetComponent<Sample_Board>().myState != nowTurn)
            {
                int targetState = board[targetX, targetY].GetComponent<Sample_Board>().myState;
                if (targetState == -1)
                {
    ...
    {
        List<Vector2> list = GetChangeList(x, y);

        if (list.Count == 0) return;

        isNext = true;
        ChangeStone(x, y, nowTurn);
        for (int i = 0; i < list.Count; i++) ChangeStone((int)list[i].x, (int)list[i].y, nowTurn);
